...
public class EventDatabase {
    ...
    public Error_Messages register(Message received_register) throws SQLException
    {
        ...
        if(user_To_Register.name.length()<2||user_To_Register.name.length()>128)
            return Error_Messages.INVALID_USERNAME;
        try {
            query = String.format("insert into utilizador (username,nome,password,administrador,autenticado) values(%s,%s,%s,false,true) ", user_To_Register.username,
                    user_To_Register.name, Utils.hashString(user_To_Register.password));
            query="insert into utilizador (username,nome,password,administrador,autenticado) values(?,?,?,false,true)" ;
    ...
    public Error_Messages changeUserData(Message change_data)
    {
        ...
        try {
            ...
            query="update utilizador set username=?,nome=?,password=? where id = ? " ;
