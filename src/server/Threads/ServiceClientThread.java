            ...
            while (!isInterrupted()) {
                ...
                while (selected_key_iterator.hasNext()) {
                    ...
                    //something came from the Client
                        ...
                        if(received_message.type_of_message == Message.TYPE_OF_MESSAGE.LOGIN){
                            ...
                            if(!((EventFilter) received_message.attachment).visibility && isUserAdmin()) {
                               ...
                               EventFilterViewModel eventFilterViewModel = (EventFilterViewModel) message_rcv.attachment;
                                ...
                                if(eventFilterViewModel.eventInfoList.isEmpty()){
                                    _is_Looking_For_Shows = false;
                                }else{
                            ...
                            List<EventInfo> shows= _server_model.getEventDatabase().getListOfAllShows(booked,_user_being_serviced.administrator);
                        ...
                        {
                            _is_Looking_For_Shows=false;
                            _is_Looking_For_Seats=false;
                            _show_Id_OF_Seat_Been_Chosen=0;
                        }
                        else if(received_message.type_of_message==Message.TYPE_OF_MESSAGE.GET_AVAILABLE_SEATS)
                        {
                            _show_Id_OF_Seat_Been_Chosen=(Integer)received_message.attachment;
                            _is_Looking_For_Shows=false;
                            _is_Looking_For_Seats=true;
                            ServerModel.sendMessageViaChannel(
                                    new Message(Message.TYPE_OF_MESSAGE.BOOK_SEAT,_server_model.getEventDatabase().get_Available_Seats(_show_Id_OF_Seat_Been_Chosen))
                                    ,_client_socket_channel
                            );
                        ...
                        {
                            SeatPrice chosen_Seat=(SeatPrice)received_message.attachment;
                            ServerModel.sendMessageViaChannel(
                                    new Message(Message.TYPE_OF_MESSAGE.BOOK_SEAT,_server_model.getEventDatabase().book_Seat(chosen_Seat,_user_being_serviced))
                                    ,_client_socket_channel
                            );
