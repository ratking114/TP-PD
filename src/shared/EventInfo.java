...
public class EventInfo implements Serializable {
    ...
    public String toString() {
        return String.format("ID:%d\nDesignação:%s\nTipo:%s\nData/Hora:%s\nDuração:%d\nLocal:%s\nLocalidade:%s\nPais:%s\nClassificação etária:%s",
                id,designation,type,date, duration,location, county, country,age_Restriction);
    }


    @Override
    public boolean equals(Object obj) {
        if(!(obj instanceof EventInfo other) || obj == this)
            return false;

        return this.location.equals(other.location) && this.date.equals(other.date) && this.designation.equals(other.designation);
    }
}