...
public class Client {
    ...
    void tryToConnectToServer(){
        ...
        //send to the client our info, our user and what we are doing
            ...
            while(selected_key_iterator.hasNext()) {
                ...
                if( ((TYPE_OF_CHANNEL)selected_channel.attachment()) == TYPE_OF_CHANNEL.PIPE){
                    ...
                    if(isUserAuthenticated() == false && (!user_command.equals("login") && !user_command.equals("register") && !user_command.equals("exit"))){
                        ...
                        getListOfShows(false).forEach(s->System.out.println(s));
                    }
    ...
    public String makeVisible(int id){
        ...
        switch (error_message){
            case SUCESS -> {return String.format("The show with the id %d is now visible",id);}
            default -> {return error_message.toString();}
    ...
    private String reserveSeat() throws IOException {
        ...
        while(true) {
            ...
            try {
                ...
                List<SeatPrice> available_Seats=( List<SeatPrice>)available_Seats_Answer.attachment;
               ...
               seat_booking_loop: while(_continue) {
                    ...
                    available_Seats.forEach(seat -> System.out.println(seat));
                    waitForKeyboardInformation();
                    ...
                    try {
                        ...
                        try{
                            selected_seat=available_Seats.stream().filter(e->e.seat==seat_Number&&e.row==row.charAt(0)).findFirst().get();
                        }
                        ...
                        sendTCPMessage(new Message(Message.TYPE_OF_MESSAGE.BOOK_SEAT,selected_seat));
                        Message booking_Result= waitAndTakeOutServerAnswer();
                        if(((Error_Messages)booking_Result.attachment)==Error_Messages.SEAT_ALEADY_BOOKED||((Error_Messages)booking_Result.attachment)==Error_Messages.SEAT_BOOKING_INFO_INVALID)
                        {
                            System.out.println(((Error_Messages)booking_Result.attachment));
                            continue;
                        }
                        else {
                            do {
                                ...
                                if (to_Continue.equalsIgnoreCase("n")) {
                                    sendTCPMessage(new Message(Message.TYPE_OF_MESSAGE.GIVE_UP_BOOKING, null));
                                    return "The user is done booking seat for this show";
                                }
                                else {
