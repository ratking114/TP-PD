        ...
        while(true){
            ...
            while(selected_key_iterator.hasNext()) {
                ...
                if( ((TYPE_OF_CHANNEL)selected_channel.attachment()) == TYPE_OF_CHANNEL.PIPE){
                    ...
                    //if it's the login fetch the required data from the user
                    else if(!isUserAuthenticated() && !user_command.equals("login")){
                        System.out.println("Error: Must login first in order to use the system");
                    }
