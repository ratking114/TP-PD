...
public class TransactionHandler extends Thread {
    ...
    public void run() {
        while(true) {
            ...
            //see if we can process the request, or in other words, if we the database is not
            //being updated
            try {
                _servermodel.getDatabaseLock().acquire();
            }catch (InterruptedException ignored){}

            System.out.println("We got the database lock!");
            ...
            //we are the only server and we can update the database directly
            else{
                //signal to the requester that everything went fine
                request.answer = Message.TYPE_OF_MESSAGE.COMMIT;
                _servermodel.getProducerConsumerBuffer().finishRequestProcessing(request);
            ...
            //signal that the database is not being updated anymore
            _servermodel.getDatabaseLock().release();
        }
